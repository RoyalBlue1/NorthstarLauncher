...
SQRESULT serverSq_DecodeJSON(void* sqvm) {
	...
	for (int i = 0; i < doc.MemberCount();i++) {
		...
		if ((long long)itr.name.GetString() == 0xcdcdcdcdcdcd) continue;
	...
	for (auto& itr : arr->GetArray()) {
		switch (itr.GetType()) {
			case rapidjson::kObjectType:
				...
				if (usedType != itr.GetType())continue;
			...
			case rapidjson::kStringType:
				...
				break;
			case rapidjson::kTrueType:
				if (usedType != rapidjson::kTrueType && usedType != rapidjson::kFalseType)continue;
				ServerSq_pushbool(sqvm, true);
				ServerSq_arrayappend(sqvm, -2);
				break;
			case rapidjson::kFalseType:
	...
	for (int i = 0; i < table->_numOfNodes; i++) {
		...
		if (node->key._Type == OT_STRING) {
			...
			case OT_STRING:
				...
				obj->AddMember(rapidjson::StringRef(node->key._VAL.asString->_val), rapidjson::StringRef(node->val._VAL.asString->_val), allocatorDoc->GetAllocator());
				break;
			case OT_INTEGER:
				obj->AddMember(rapidjson::StringRef(node->key._VAL.asString->_val), rapidjson::Value((int)node->val._VAL.asInteger), allocatorDoc->GetAllocator());
				break;
			case OT_FLOAT:

				obj->AddMember(rapidjson::StringRef(node->key._VAL.asString->_val), rapidjson::Value(node->val._VAL.asFloat), allocatorDoc->GetAllocator());
				break;
			case OT_BOOL:
				if (node->val._VAL.asInteger) {
					obj->AddMember(rapidjson::StringRef(node->key._VAL.asString->_val), rapidjson::Value(true), allocatorDoc->GetAllocator());
				}
				else {
					obj->AddMember(rapidjson::StringRef(node->key._VAL.asString->_val), rapidjson::Value(false), allocatorDoc->GetAllocator());
				}
				break;
			case OT_TABLE:
